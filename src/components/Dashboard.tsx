...
import { Search, History, MousePointer2, Loader2, PanelLeftClose, PanelLeftOpen, UserCircle2, ChevronRight, Trash2, CheckSquare, Square, X } from "lucide-react";
...
export function Dashboard({ userName, greeting }: { userName: string, greeting: string }) {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-bg-workspace relative">
      {/* Sidebar: History */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-[60] lg:z-10 border-r border-border-subtle bg-bg-base flex flex-col shrink-0 transition-all duration-300 ${isHistoryOpen ? "translate-x-0 w-[85%] sm:w-72" : "-translate-x-full lg:translate-x-0 lg:w-0 lg:overflow-hidden lg:border-none"}`}>
        <div className="px-6 pt-8 pb-4 flex justify-between items-center whitespace-nowrap">
          ...
          <button onClick={() => setIsHistoryOpen(false)} className="lg:hidden p-2 text-text-secondary">
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <button 
        ...
        className={`absolute top-4 sm:top-10 z-[56] lg:z-50 p-2 bg-bg-base border border-border-subtle rounded-md shadow-sm text-text-secondary hover:text-text-primary transition-all duration-300 ${isHistoryOpen ? "left-[calc(85%-16px)] sm:left-[274px]" : "left-4 lg:left-6"}`}
      >
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300`}>
        <div className="px-4 sm:px-6 lg:px-12 pt-8 lg:pt-10 pb-3 shrink-0">
        ...
        <div className="px-4 sm:px-6 lg:px-12 pb-6 shrink-0"><CommandBar /></div>
        
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-12 pb-12 space-y-6 sm:space-y-8 custom-scrollbar">
          {showMonitor && <ProcessMonitor campaignId={activeTab} />}

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
            <div className="bg-bg-base border border-border-subtle p-5 sm:p-8 group hover:border-brand-accent/30 transition-all relative overflow-hidden">
              <span className="text-[9px] sm:text-[10px] text-text-secondary uppercase tracking-[0.2em] font-bold block mb-2">Total Leads</span>
              <span className="text-3xl sm:text-4xl font-light text-text-primary tabular-nums">{totalLeads}</span>
              <Search className="absolute -right-2 -bottom-2 w-16 sm:w-24 h-16 sm:h-24 opacity-[0.03] group-hover:opacity-[0.06]" />
            </div>
            <div className="bg-bg-base border border-border-subtle p-5 sm:p-8 group hover:border-brand-accent/30 transition-all relative overflow-hidden">
              <span className="text-[9px] sm:text-[10px] text-text-secondary uppercase tracking-[0.2em] font-bold block mb-2">Conversions</span>
              <span className="text-3xl sm:text-4xl font-light text-brand-accent tabular-nums">{totalConversions}</span>
              <MousePointer2 className="absolute -right-2 -bottom-2 w-16 sm:w-24 h-16 sm:h-24 opacity-[0.03] group-hover:opacity-[0.06]" />
            </div>
          </div>

          <div className="bg-bg-base border border-border-subtle relative overflow-hidden flex flex-col">
            {/* Bulk Toolbar */}
            <div className={`absolute top-0 inset-x-0 h-auto min-h-[64px] bg-text-primary text-white flex flex-col sm:flex-row items-center justify-between px-5 lg:px-8 py-3 sm:py-0 transition-all duration-300 transform ${selectedIds.length > 0 ? "translate-y-0 opacity-100 z-10" : "-translate-y-full opacity-0 pointer-events-none z-0"}`}>
               <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 w-full sm:w-auto">
                 ...
                 <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 no-scrollbar">
                    <button onClick={() => handleBulkOutreach("email")} className="flex-1 sm:flex-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest bg-white/10 hover:bg-white/20 px-3 sm:px-4 py-2 border border-white/20 transition-colors text-white whitespace-nowrap">Email</button>
                    <button onClick={() => handleBulkOutreach("whatsapp")} className="flex-1 sm:flex-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest bg-white/10 hover:bg-white/20 px-3 sm:px-4 py-2 border border-white/20 transition-colors text-white whitespace-nowrap">WhatsApp</button>
                    <button onClick={() => handleBulkOutreach("both")} className="flex-1 sm:flex-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest bg-brand-accent text-white px-4 py-2 hover:scale-105 active:scale-95 transition-transform whitespace-nowrap">Send All</button>
                 </div>
               </div>
               <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t border-white/10 sm:border-none">
                  <button className="text-[9px] sm:text-[10px] uppercase font-bold text-red-400 hover:text-red-300" onClick={handleBulkDeleteLeads}><Trash2 className="w-3.5 h-3.5 mr-2 inline" />Delete</button>
                  <button className="text-[9px] sm:text-[10px] uppercase font-bold opacity-60 hover:opacity-100" onClick={() => setSelectedIds([])}>Cancel</button>
               </div>
            </div>

            {/* List Header */}
            <div className="px-4 sm:px-8 py-4 sm:py-5 border-b border-border-subtle flex flex-col sm:flex-row sm:items-center justify-between bg-bg-base/80 gap-4">
              <div className="flex items-center justify-between sm:justify-start gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-text-primary truncate max-w-[150px] sm:max-w-none">
              ...
              <div className="flex bg-bg-workspace p-1 border border-border-subtle rounded-sm h-9 sm:h-8 overflow-x-auto no-scrollbar">
                 <button onClick={() => setFilterMode("all")} className={`flex-1 sm:flex-none px-3 text-[9px] font-bold uppercase tracking-widest rounded-sm whitespace-nowrap ${filterMode === "all" ? "bg-bg-base text-text-primary shadow-sm" : "text-text-secondary"}`}>All</button>
                 <button onClick={() => setFilterMode("email")} className={`flex-1 sm:flex-none px-3 text-[9px] font-bold uppercase tracking-widest rounded-sm whitespace-nowrap ${filterMode === "email" ? "bg-bg-base text-brand-accent shadow-sm" : "text-text-secondary"}`}>Email</button>
                 <button onClick={() => setFilterMode("whatsapp")} className={`flex-1 sm:flex-none px-3 text-[9px] font-bold uppercase tracking-widest rounded-sm whitespace-nowrap ${filterMode === "whatsapp" ? "bg-bg-base text-brand-accent shadow-sm" : "text-text-secondary"}`}>WhatsApp</button>
              </div>
            </div>
            ...
            <div className="divide-y divide-border-subtle">
              {leads.filter(l => {
                ...
                return (
                  <div key={lead.id} className={`flex flex-col sm:flex-row sm:items-center justify-between px-4 sm:px-8 py-4 sm:py-5 hover:bg-bg-workspace group transition-colors gap-3 sm:gap-4 ${selectedIds.includes(lead.id) ? "bg-bg-workspace/50" : ""}`}>
                    <div className="flex items-center gap-4 sm:gap-5 flex-1 min-w-0">
                      ...
                      >
                        {selectedIds.includes(lead.id) ? (
                          ...
                          <Square className="w-4 h-4 opacity-40 sm:opacity-0 group-hover:opacity-100" />
                        )}
                      </button>
                      <div className="flex items-center gap-3 sm:gap-5 flex-1 min-w-0 cursor-pointer" onClick={() => setSelectedLead(lead.id)}>
                        <div className="w-9 h-9 sm:w-10 sm:h-10 bg-bg-workspace border border-border-subtle flex items-center justify-center shrink-0">
                          <span className="text-xs sm:text-sm font-bold text-text-secondary group-hover:text-brand-accent">{lead.first_name?.[0]?.toUpperCase() || "?"}</span>
                        </div>
                        <div className="min-w-0">
                          <div className="text-xs sm:text-sm font-medium text-text-primary group-hover:text-brand-accent truncate">{lead.first_name} {lead.last_name}</div>
                          <div className="text-[10px] sm:text-[11px] text-text-secondary mt-0.5 truncate">{lead.company || "Unknown"}</div>
                        </div>
                      </div>
                    </div>
                    
                    <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0 ml-8 sm:ml-0">
                      <div className="flex items-center gap-2">
                        {he && <span className="text-[8px] sm:text-[9px] font-bold uppercase px-1.5 sm:px-2 py-0.5 sm:py-1 bg-blue-50 text-blue-600 border border-blue-200 whitespace-nowrap">✉ Email</span>}
                        {hp && <span className="text-[8px] sm:text-[9px] font-bold uppercase px-1.5 sm:px-2 py-0.5 sm:py-1 bg-green-50 text-green-600 border border-green-200 whitespace-nowrap">📱 Mobile</span>}
                      </div>

                      <div className="flex items-center gap-3">
                        <span className="text-[9px] sm:text-[10px] text-text-secondary font-medium uppercase hidden md:block">{lead.score}%</span>
                        <span className={`text-[8px] sm:text-[10px] font-bold uppercase px-1.5 sm:px-2 py-0.5 border ${lead.status === 'replied' ? 'bg-brand-accent border-brand-accent text-white' : 'border-border-subtle text-text-secondary'}`}>{lead.status}</span>
                        {(he || hp) && lead.status !== 'contacted' && (
                          <button onClick={(e) => { e.stopPropagation(); setSelectedLead(lead.id); }} className="text-[9px] sm:text-[10px] font-bold uppercase bg-text-primary text-white px-3 py-1 hover:bg-brand-accent transition-all opacity-100 sm:opacity-0 group-hover:opacity-100">Send</button>
                        )}
                      </div>
                    </div>
                  </div>
